...
typedef struct BucketType          /* Bucket structure for RadixSort */
{
   long *num;                      /* array of #'s in bucket  */
   int count;                      /* number of #'s in bucket */
...
{
   int i, j, k;              /* index of for loops */
   int counter = 0;          /* Used to count elements for list */
   unsigned long MaskValue;  /* stores value after number is masked */
   unsigned long MASK = 0x0000000F; /* Mask for bucket distribution */
   BUCKET bucket[16];       /* 16 buckets to hold 4-bit digits */
   int NUMFIELDS = 8;       /* number of fields in each number */
...
/* Creates an array of size n for Radix Sort data structure BUCKET. */
{
   BUCKET b;
...
void InitializeBucket(BUCKET *b)
/* Initializes the bucket to hold 0 values */
{
   b->count = 0;   /* set the bucket value count to 0 */
}
...
void ShowResults(char *string, float t1, float t2, int n, char *filename)
{
  long micro = t2 - t1;               /* time in microseconds */
  float sec = micro / CLOCKS_PER_SEC;  /* time in seconds */
  FILE *file;
